...
(() => {
  ...
  // ── База знаний ──
  const knowledge = [
    { id: 'kk1', cat: 'Скрипт',    title: 'Скрипт первого звонка по заявке',          body: 'Поздороваться, уточнить бюджет и район, спросить про срочность и тип сделки (продажа/аренда), предложить 2-3 подходящих варианта из каталога, назначить показ на конкретное время.' },
    { id: 'kk2', cat: 'Регламент', title: 'Регламент показа объекта',                 body: 'Приехать на 10 минут раньше, проверить доступ к квартире заранее, пройтись по чек-листу вопросов клиента (что нравится/не нравится), зафиксировать обратную связь в карточке лида сразу после показа.' },
    { id: 'kk3', cat: 'Инструкция',title: 'Как оформить задаток',                     body: 'Соглашение о задатке — 2-5% от суммы сделки, вносится наличными или переводом на счёт агентства, срок действия 7-14 дней. Обязательно зафиксировать в разделе «Документы» до внесения.' },
    { id: 'kk4', cat: 'Регламент', title: 'Если клиент не выходит на связь',          body: 'Три попытки дозвона за 2 дня в разное время, затем сообщение в WhatsApp. Если тишина — перевести лид в статус «холодный», из базы не удалять, вернуться через месяц.' },
  ];
  ...
  window.RECIPE = {
    ...
    metrics: [],
    seed: { finance, tasks, docs, knowledge },
  };
})();
