...
(() => {
  ...
  // (30 июля минус 5 месяцев давало 2 марта) — февраль выпадал, март задваивался
  const monthAgo = m => { const d = new Date(); d.setDate(1); d.setMonth(d.getMonth() - m); d.setDate(10); return d.toISOString().slice(0, 10); };
  ...
  const APPOINTMENTS_RAW = [
    ['Лечение кариеса 2.6 — Алексей Морозов',        'p1',  'Петрова О.А.',      ds(-24), 4500,  'confirmed', 'Терапия', 45],
    ['Профгигиена Air Flow — Светлана Ковалёва',     'p2',  'Осипова Е.Н.',      ds(-23), 5500,  'done', 'Гигиена', 45],
    ['Имплантация (этап 1) — Дмитрий Федотов',       'p3',  'Рогов И.С.',        ds(-22), 38000, 'paid', 'Хирургия', 60],
    ['Брекет-система, установка — Мария Зайцева',    'p4',  'Осипова Е.Н.',      ds(-21), 45000, 'confirmed', 'Ортодонтия', 60],
    ['Профгигиена + фторирование — Ирина Белова',    'p5',  'Осипова Е.Н.',      ds(-20), 6200,  'paid', 'Гигиена', 45],
    ['Панорамный снимок ОПТГ — Николай Орлов',        'p6',  'Рогов И.С.',        ds(-19), 1500,  'paid', 'Диагностика', 20],
    ['Осмотр + план лечения — Екатерина Соколова',   'p7',  'Петрова О.А.',      ds(-17), 1000,  'new', 'Диагностика', 30],
    ['Удаление зуба мудрости 4.8 — Павел Громов',    'p8',  'Рогов И.С.',        ds(-16), 8500,  'confirmed', 'Хирургия', 45],
    ['Отбеливание ZOOM — Анна Лебедева',             'p9',  'Осипова Е.Н.',      ds(-15), 22000, 'done', 'Гигиена', 60],
    ['Коронка металлокерамика 2.7 — Сергей Тимофеев','p10', 'Климов И.С.',       ds(-14), 18000, 'paid', 'Протезирование', 60],
    ['Пломба световая 3.5 — Ольга Кузнецова',        'p11', 'Петрова О.А.',      ds(-13), 4800,  'confirmed', 'Терапия', 45],
    ['Лечение пародонтита — Виктор Симонов',         'p12', 'Петрова О.А.',      ds(-12), 7400,  'paid', 'Терапия', 45],
    ['Виниры, консультация — Татьяна Романова',      'p13', 'Климов И.С.',       ds(1),         2000,  'confirmed', 'Протезирование', 30],
    ['Реставрация скола 1.1 — Андрей Захаров',       'p14', 'Петрова О.А.',      ds(2),         5600,  'new', 'Терапия', 45],
    ['Отбеливание + гигиена — Юлия Никитина',        'p15', 'Осипова Е.Н.',      ds(3),         24000, 'confirmed', 'Гигиена', 60],
    ['Имплант + коронка 4.6 — Игорь Воронов',        'p16', 'Рогов И.С.',        ds(5),         42000, 'confirmed', 'Хирургия', 60],
    ['Лечение кариеса, повтор — Алексей Морозов',    'p1',  'Петрова О.А.',      ds(7),         4200,  'new', 'Терапия', 45],
    ['Имплантация (этап 2) — Дмитрий Федотов',       'p3',  'Рогов И.С.',        ds(10),        26000, 'new', 'Хирургия', 60],
  ];
  const appointments = APPOINTMENTS_RAW.map((d, i) => ({
    id: 'a' + (i + 1),
    name: d[0], patientId: d[1], doctor: d[2], date: d[3], price: d[4],
    // услуга и длительность заданы у каждого приёма: раньше они раздавались по кругу
    // (i % 6 / i % 4), и «Удаление зуба мудрости» попадало в «Гигиену», а «Отбеливание» — в «Хирургию»
    service: d[6], duration: d[7],
    stage: d[5],
    notes: '',
    createdAt: dt(-(i * 2 + 1)),
  ...
  // ── Счета ──
  const invoices = [
    { id: 'inv1', name: 'Счёт №001 — Имплантация Федотов', patient: 'Дмитрий Федотов', amount: 38000, status: 'paid',    date: ds(-22), service: 'Имплантация (этап 1)', stage: 'paid' },
    { id: 'inv2', name: 'Счёт №002 — Пародонтит Симонов', patient: 'Виктор Симонов', amount: 7400, status: 'paid', date: ds(-12), service: 'Лечение пародонтита', stage: 'paid' },
    { id: 'inv3', name: 'Счёт №003 — Гигиена Белова', patient: 'Ирина Белова', amount: 6200, status: 'paid', date: ds(-20), service: 'Профгигиена + фторирование', stage: 'paid' },
    ...
    { id: 'inv6', name: 'Счёт №006 — Коронка Тимофеев', patient: 'Сергей Тимофеев', amount: 18000, status: 'paid', date: ds(-14), service: 'Коронка металлокерамика', stage: 'paid' },
    { id: 'inv7', name: 'Счёт №007 — Диагностика Орлов', patient: 'Николай Орлов', amount: 1500, status: 'paid', date: ds(-19), service: 'Панорамный снимок ОПТГ', stage: 'paid' },
  ...
  window.RECIPE = {
    ...
    // подписи движка под нишу: по умолчанию они из недвижимости («Скоро освобождается»,
    // «Зависшие сделки»), и в стоматологической CRM это читалось как чужой текст
    lexicon: { upcomingOne: 'Скоро приём', upcomingMany: 'Ближайшие приёмы', stuckMany: 'Без движения' },
    ...
    seed: {
      ...
      tasks: [
        ...
        { id: 'ct6', title: 'Закупить расходники: боры, слюноотсосы, перчатки', due: ds(2), done: false },
      ...
      team: [
        { id: 'tm1', name: 'Оксана Петрова',  role: 'owner',     contact: '@o_petrova',    note: 'Главврач, стоматолог-терапевт', kpi: { team: 8, revenue: 980000, target: 1200000 } },
      ...
      reference: {
        services: [
          ...
          { id: 'cs12',name: 'Панорамный снимок ОПТГ',    dept: 'Диагностика',    price: 1500,  duration: 15 },
        ],
      },
    },
  };
