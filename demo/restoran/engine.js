...
function ava(name, sm) { if (!name) return ''; const n = String(name).trim(); const i = (n.replace(/[«»"']/g, '')[0] || '?').toUpperCase(); let h = 0; for (const c of n) h = (h * 33 + c.charCodeAt(0)) % 360; return `<span class="ava${sm ? ' ava--sm' : ''}" style="background:hsl(${h},52%,52%)" title="${escAttr(n)}">${esc(i)}</span>`; }
...
function viewKanban(n) {
  const ent = entById(n.entity), recs = DB.recs(ent.key);
  // сумма сделки/заказа, а не производная прибыль: computed искался первым, поэтому
  // колонка «Доставлен» показывала 7 850 ₽ прибыли там, где в таблице стоит 12 340 ₽
  const moneyF = ent.fields.find(f => f.type === 'money') || ent.fields.find(f => f.type === 'computed');
...
function openForm(ek, id, preset) {
  ...
  const inputs = ent.fields.filter(f => f.type !== 'computed').map(f => {
    ...
    else ctrl = `<input class="t-input t-input--full" id="f_${f.key}" type="${(f.type === 'money' || f.type === 'number') ? 'number' : f.type === 'date' ? 'date' : 'text'}" value="${escAttr(rec[f.key] || '')}" ${f.required ? 'required' : ''}>`;
...
function openPayment(id) {
  ...
  mount(`<div class="overlay overlay--center" id="ov"><div class="panel panel--modal"><div class="panel__head"><div class="panel__title">${id ? 'Изменить платёж' : 'Новый платёж'}</div><button class="icon-btn" id="pClose">${icon('x')}</button></div><form class="panel__body" id="payForm">
    <div class="f-col"><label class="f-label">Назначение *</label><input class="t-input t-input--full" id="p_title" value="${escAttr(p.title || '')}" required></div>
    <div class="f-row"><div class="f-col"><label class="f-label">Сумма (₽) *</label><input class="t-input t-input--full" type="number" id="p_amount" value="${escAttr(p.amount || '')}" required></div><div class="f-col"><label class="f-label">Тип</label><input class="t-input t-input--full" id="p_kind" value="${escAttr(p.kind || '')}" placeholder="Аренда / Депозит"></div></div>
    <div class="f-row"><div class="f-col"><label class="f-label">Срок</label><input class="t-input t-input--full" type="date" id="p_due" value="${escAttr(p.due || '')}"></div><div class="f-col"><label class="f-label">Статус</label><select class="t-select t-select--full" id="p_status">${['draft', 'sent', 'paid', 'overdue'].map(s => `<option value="${s}" ${p.status === s ? 'selected' : ''}>${({ draft: 'Черновик', sent: 'Выставлен', paid: 'Оплачен', overdue: 'Просрочен' })[s]}</option>`).join('')}</select></div></div>
...
function openFin(id, presetType) {
  ...
  mount(`<div class="overlay overlay--center" id="ov"><div class="panel panel--modal"><div class="panel__head"><div class="panel__title">${id ? 'Операция' : (t.type === 'expense' ? 'Новый расход' : 'Новый доход')}</div><button class="icon-btn" id="fnClose" aria-label="Закрыть">${icon('x')}</button></div><form class="panel__body" id="finForm">
    ...
    <div class="f-col"><label class="f-label">Назначение *</label><input class="t-input t-input--full" id="fn_title" value="${escAttr(t.title || '')}" placeholder="Напр.: Выкуп студии «Око 41»" required></div>
    <div class="f-grid2"><div class="f-col"><label class="f-label">Категория</label><input class="t-input t-input--full" id="fn_cat" list="finCats" value="${escAttr(t.category || '')}"><datalist id="finCats">${cats.map(c => `<option value="${escAttr(c)}">`).join('')}</datalist></div>
    <div class="f-col"><label class="f-label">Сумма (₽) *</label><input class="t-input t-input--full" type="number" id="fn_amount" value="${escAttr(t.amount || '')}" required></div></div>
    <div class="f-col"><label class="f-label">Дата</label><input class="t-input t-input--full" type="date" id="fn_date" value="${escAttr(t.date || todayStr())}"></div>
...
function viewGoals(n) {
  ...
  // у части команды (управляющий, повар, маркетолог) выручки нет по определению —
  // показывали «0 ₽ NaN%». Сортируем и считаем только тех, у кого выручка есть
  // личная цель тем, у кого её нет в рецепте: доля общей цели на человека с выручкой.
  // Раньше брали 80% от максимума по команде — курьеру с 4к ставилась цель 736к и он висел на 1%
...
function openDoc(id) {
  ...
  mount(`<div class="overlay overlay--center" id="ov"><div class="panel panel--modal"><div class="panel__head"><div class="panel__title">${id ? 'Документ' : 'Новый документ'}</div><button class="icon-btn" id="dcClose">${icon('x')}</button></div><form class="panel__body" id="docForm">
    <div class="f-col"><label class="f-label">Название *</label><input class="t-input t-input--full" id="d_title" value="${escAttr(d.title || '')}" required></div>
    <div class="f-row"><div class="f-col"><label class="f-label">Тип</label><select class="t-select t-select--full" id="d_type">${Object.entries(TYP).map(([k, v]) => `<option value="${k}" ${d.type === k ? 'selected' : ''}>${esc(v)}</option>`).join('')}</select></div><div class="f-col"><label class="f-label">Сумма (₽)</label><input class="t-input t-input--full" type="number" id="d_amount" value="${escAttr(d.amount || '')}"></div></div>
    <div class="f-row"><div class="f-col"><label class="f-label">Дата</label><input class="t-input t-input--full" type="date" id="d_date" value="${escAttr(d.date || '')}"></div><div class="f-col"><label class="f-label">Статус</label><select class="t-select t-select--full" id="d_status">${['draft', 'sent', 'signed', 'paid'].map(s => `<option value="${s}" ${d.status === s ? 'selected' : ''}>${({ draft: 'Черновик', sent: 'Отправлен', signed: 'Подписан', paid: 'Оплачен' })[s]}</option>`).join('')}</select></div></div>
    ...
    <div class="f-col" data-att="link"><label class="f-label">Ссылка на документ</label><input class="t-input t-input--full" id="d_attUrl" placeholder="https://…" value="${escAttr(d.attach && d.attach.url || '')}"></div>
...
function calNav(viewLabel, title) {
  ...
  <div class="cal-tools"><div class="cal-datewrap">${icon('calendar')}<input type="date" id="calDate" value="${escAttr(CAL_SEL)}"></div><div class="cal-searchwrap">${icon('search')}<input id="calSearch" placeholder="Поиск по задачам…" value="${escAttr(CAL_Q || '')}"></div></div>`;
}
function calBind() {
...
function openTask(id, preset) {
  ...
  const team = DB.team();
  const asgOpts = '<option value="">— общая (без исполнителя)</option>' + team.map(m => `<option value="${escAttr(m.name)}" ${t.assignee === m.name ? 'selected' : ''}>${esc(m.name)} · ${esc(roleName(m.role))}</option>`).join('');
  mount(`<div class="overlay overlay--center" id="ov"><div class="panel panel--modal"><div class="panel__head"><div class="panel__title">${id ? 'Задача' : 'Новая задача'}</div><button class="icon-btn" id="tkClose">${icon('x')}</button></div><form class="panel__body" id="tkForm">
    <div class="f-col"><label class="f-label">Что сделать *</label><input class="t-input t-input--full" id="tk_title" value="${escAttr(t.title || '')}" required></div>
    <div class="f-row"><div class="f-col"><label class="f-label">Дата</label><input class="t-input t-input--full" type="date" id="tk_date" value="${escAttr(t.due || preset.date || CAL_SEL)}"></div><div class="f-col" style="justify-content:flex-end"><label class="chk"><input type="checkbox" id="tk_allday" ${allday ? 'checked' : ''}> Весь день</label></div></div>
    <div class="f-row" id="tk_times" style="${allday ? 'display:none' : ''}"><div class="f-col"><label class="f-label">Начало</label><input class="t-input t-input--full" type="time" id="tk_start" value="${escAttr(t.start || preset.start || '10:00')}"></div><div class="f-col"><label class="f-label">Конец</label><input class="t-input t-input--full" type="time" id="tk_end" value="${escAttr(t.end || '')}"></div></div>
...
function openMember(id) {
  ...
  mount(`<div class="overlay overlay--center" id="ov"><div class="panel panel--modal"><div class="panel__head"><div class="panel__title">${id ? 'Сотрудник' : 'Новый сотрудник'}</div><button class="icon-btn" id="mClose">${icon('x')}</button></div><form class="panel__body" id="memForm">
    <div class="f-col"><label class="f-label">Имя *</label><input class="t-input t-input--full" id="m_name" value="${escAttr(m.name || '')}" required></div>
    <div class="f-row"><div class="f-col"><label class="f-label">Роль</label><select class="t-select t-select--full" id="m_role">${roleOpts}</select></div><div class="f-col"><label class="f-label">Контакт</label><input class="t-input t-input--full" id="m_contact" value="${escAttr(m.contact || '')}" placeholder="@tg / телефон"></div></div>
    ...
    <div id="m_kpis"></div><div class="f-col"><label class="f-label">Заметка</label><input class="t-input t-input--full" id="m_note" value="${escAttr(m.note || '')}"></div>
...
function viewHub() {
  ...
  // без отменённых: «средний чек» и «прибыль» на этом экране считались вместе с отменённым
  // заказом и расходились с теми же показателями на Живой кухне и в Аналитике
  const orders = (typeof DB !== 'undefined' && DB.recs) ? DB.recs('order').filter(o => o.stage !== 'cancelled') : [];
...
function viewKitchen() {
        ...
        ${COLS.map(c => { const list = orders.filter(o => o.stage === c.key);
          return `<div class="kb-col${c.key === 'cooking' ? ' kb-col--live' : ''}" style="--kc:${c.col}">
            ...
            <div class="kb-col__body">
              ${list.map(o => `<div class="kb-ticket" data-id="${o.id}" data-ent="order">
  ...
  // тикеты кухни открываются, как любая другая запись: ховер их приподнимает,
  // то есть интерфейс обещает клик — раньше клик не делал ничего
  bindRowClicks();
}
...
function openAutomation(id) {
  ...
  mount(`<div class="overlay overlay--center" id="ov"><div class="panel panel--modal"><div class="panel__head"><div class="panel__title">${id ? 'Сценарий' : 'Новый сценарий'}</div><button class="icon-btn" id="aClose">${icon('x')}</button></div><form class="panel__body" id="autoForm">
    <div class="f-col"><label class="f-label">Название</label><input class="t-input t-input--full" id="a_name" value="${escAttr(a.name || '')}" placeholder="Напр.: Новый лид → задача менеджеру"></div>
    ...
    <div class="f-grid2">
      <div class="f-col"><label class="f-label">Текст задачи / документа</label><input class="t-input t-input--full" id="a_taskTitle" value="${escAttr(a.taskTitle || '')}" placeholder="Напр.: Связаться с клиентом"></div>
...
function openKb(id) {
  ...
  mount(`<div class="overlay overlay--center" id="ov"><div class="panel panel--modal"><div class="panel__head"><div class="panel__title">${id ? 'Статья' : 'Новая статья'}</div><button class="icon-btn" id="kClose">${icon('x')}</button></div><form class="panel__body" id="kbForm">
    <div class="f-col"><label class="f-label">Заголовок *</label><input class="t-input t-input--full" id="k_title" value="${escAttr(a.title || '')}" required></div>
    <div class="f-col"><label class="f-label">Категория</label><input class="t-input t-input--full" id="k_cat" value="${escAttr(a.cat || '')}" placeholder="Регламент / Скрипт / Инструкция"></div>
...
function viewSettings() {
  ...
  document.getElementById('viewBody').innerHTML = `<div class="settings-grid">
    <div class="bcard"><div class="bcard__head"><span class="bcard__label">Компания</span></div><div class="f-col"><label class="f-label">Название</label><input class="t-input t-input--full" id="set_brand" value="${escAttr(R.brand?.name || '')}"></div><button class="btn" id="setSave" style="margin-top:10px">Сохранить</button></div>
