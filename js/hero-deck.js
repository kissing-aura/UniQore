...
(function () {
  ...
  панели.forEach(function (панель) {
    ...
    if (!время) return;
    /* Живая цифра тянет за собой ВСЁ, что от неё считается. Иначе через три
       минуты KPI показывает 54 заявки, а «Источники» под ним по-прежнему
       22+15+10+6 = 53, и стоимость заявки остаётся от старого делителя.
       Растёт Instagram — самый крупный канал, полоса и так стоит на 100%. */
    var инстК = окно.querySelector('[data-view="dash"] .src li:first-child .src__v');
    var ценаК = окно.querySelector('[data-view="dash"] .src__foot b');
    var РАСХОД = 364500;
    var мин = 2, заявок = 53, оплат = 18, инст = 22;
    setInterval(function () {
      ...
      заявок += 1;
      инст += 1;
      if (инстК) инстК.textContent = инст;
      if (ценаК) ценаК.firstChild.nodeValue = Math.round(РАСХОД / заявок).toLocaleString('ru-RU').replace(/ /g, ' ');
      if (заявкиК) {
